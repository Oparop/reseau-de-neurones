...
import type { Evaluation, MLP } from "./mlp";
...
export interface Ui {
  ...
  updateReadouts(net: MLP, evaluation: Evaluation): void;
...
export function setupUi(handlers: UiHandlers): Ui {
  ...
  const lossEl = byId<HTMLElement>("loss");
  ...
  const ui: Ui = {
    ...
    updateReadouts(net, evaluation) {
      archEl.textContent = net.sizes.join(" → ");
      paramsEl.textContent = String(net.parameterCount);
      lossEl.textContent = Number.isNaN(evaluation.loss) ? "—" : evaluation.loss.toFixed(3);
      iterEl.textContent = String(net.iterations);
      errEl.textContent = String(evaluation.errors);
    },
